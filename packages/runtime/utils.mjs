...
export function *inlineTexts(element, shouldTraverse = element => true) {
    ...
    for (let child of walk(element,
        ...
        if (child.nodeType === child.TEXT_NODE) {
            // Use textContent() rather than wholeText(): every DOM we
            // support implements it, and the result is the same on
            // text nodes. It also spares us normalizing the DOM tree
            // first.
