...
export const DEFAULT_USER_AGENT = 'SmeltCorpusBot/1.0';
export const DEFAULT_OBSERVATION_MS = 1500;
...
export function normalizeBrowserName(name) {
    const value = requireString(name ?? 'chromium', 'browser.name');
    if (value !== 'chromium' && value !== 'firefox') {
        throw new TypeError('browser.name must be "chromium" or "firefox".');
    }
    return value;
}
...
export function normalizeNonNegativeNumber(value, fallback, name) {
...
export function normalizeStorageState(value, name) {
    if (value == null) return null;
...
export function assertUniquePageIds(pages) {
    const seen = new Map();
    pages.forEach((page, index) => {
        const key = safeFilename(page.id);
        if (seen.has(key)) {
            throw new TypeError(`config.pages[${index}].id "${page.id}" collides with ` +
                `config.pages[${seen.get(key)}].id. Set unique ids.`);
        }
        seen.set(key, index);
    });
}
